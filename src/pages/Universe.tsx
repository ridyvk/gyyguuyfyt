...
const pageSize = 12
const initialFilter: CompanyFilter = {
  query: '',
  market: 'all',
  industry: 'all',
  theme: 'all',
...
export default function Universe() {
  const { companies, isWatched, toggleWatchlist } = useApp()
  const [filter, setFilter] = useState(initialFilter)
  const [page, setPage] = useState(1)
  const deferredQuery = useDeferredValue(filter.query)
  const filtered = useMemo(
    () => filterCompanies(companies, { ...filter, query: deferredQuery }),
    [companies, filter, deferredQuery],
  )
    ...
    <div className="page">
      <header className="page-header page-header--split">
        <div>
          ...
          <h1>企業を探す</h1>
          <p>軽量な一覧から、気になる変化と分析対象を絞り込みます。</p>
        </div>
        ...
        <section className="universe-results">
          <div className="results-toolbar">
            ...
            <div className="results-toolbar__note">
              ...
              {Number(listedCompanySource.date.slice(4, 6))}月末 / KPIはモック
            </div>
          </div>
