...
const isBest = (
  ...
  const values = companies.map((item) => item.metrics[key].value)
...
export default function Compare() {
  ...
  const availableCompanies = companies.filter(
    (company) => !compareList.includes(company.id),
  )
        ...
        <>
          <section className="compare-chart-grid">
            ...
            <article className="panel">
              ...
              <RadarScoreChart companies={comparedCompanies} height={360} />
            </article>
          </section>
                ...
                <article className="comparison-card" key={company.id}>
                  ...
                  <div className="comparison-card__scores">
                    {scoreMetrics.slice(1).map((metric) => {
                      ...
                      const best = score === Math.max(...comparedCompanies.map((item) => item.scores[metric.key]))
                      return <div key={metric.key}><span>{metric.label}</span><strong>{Math.round(score)}</strong>{best && <b>Best</b>}</div>
                    })}
                  </div>
                  ...
                  <div className="comparison-card__warnings">
                    <span>注意フラグ {company.warnings.length}件</span>
