...
- PER/PBRは日足終値とEPS/BPSが取得できた企業から順次表示します。
...
Yahoo Financeの公開チャートJSONから日足終値を取得し、EDINET・TDnetのXBRLから取得したEPS/BPSと組み合わせて、最新終値、前日比、PER・PBRおよびバリュエーションスコアを自動再計算します。

追加のAPIキーやSecret設定は不要です。GitHub Actionsの `Update EDINET financials` が3時間ごとに株価と財務データを更新します。
